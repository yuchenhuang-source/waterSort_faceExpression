...
const configPath = path.join(projectRoot, 'src', 'game', 'config', 'output-config.json');
...
// 获取当前配置（添加 X-Config-Source 便于调试端口冲突）
app.get('/api/constants', (req, res) => {
  try {
    const data = fs.readFileSync(configPath, 'utf8');
    const json = JSON.parse(data);
    res.setHeader('X-Config-Source', 'arrow-playable');
    res.json(json);
  } catch (err) {
  ...
  <script>
    ...
    function render() {
      ...
      for (const cat in CATEGORIES) {
        const keys = CATEGORIES[cat].filter(function(k) { return getValueByPath(data, k) !== undefined; });
