      ...
      if (id === resolvedVirtualModuleId) {
        try {
          ...
          return `
            ...
            // 将 base64 data URL 转换为 Blob URL（用于 Phaser 音频加载）
            function dataUrlToBlobUrl(dataUrl, mimeType) {
              ...
              try {
                ...
                if (!base64Data) {
                  console.warn('[enhanced-auto-assets] 无效的 data URL:', dataUrl.substring(0, 50));
                  return dataUrl;
                }
                ...
                console.error('[enhanced-auto-assets] 转换 data URL 失败:', error);
                return dataUrl;
              }
            }
            ...
            export function loadAllAssets(scene) {
              ...
              Object.entries(assets).forEach(([key, asset]) => {
                if (asset.type === 'image') {
                  ...
                  // Phaser 不支持 base64 data URL，需要转换为 Blob URL
                  let audioUrl = asset.url;
                  if (asset.url && asset.url.startsWith('data:audio/')) {
                    audioUrl = dataUrlToBlobUrl(asset.url, asset.mimeType || 'audio/mpeg');
                    console.log(\`[enhanced-auto-assets] 音频 \${key} 已转换为 Blob URL\`);
                  }
                  scene.load.audio(key, audioUrl);
