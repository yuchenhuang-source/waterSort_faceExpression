...
| 文件 | 说明 |
|------|------|
| `game/render/CvColorCode.ts` | 核心 API：CvTintable、applyCvTintables、ICvTintableProvider、ICvRenderable |
| `game/render/ObjectIdPipeline.ts` | 颜色映射：generateColorMap、extractValidPixels、ColorMap 类型 |
| `game/cv-bridge/CVBridge.ts` | WebSocket 连接 CV 服务 |
| `game/cv-integration/CvIntegration.ts` | 一键接入：S/R 键、截帧发送、状态文案 |

### 1.2 依赖关系

```
ObjectIdPipeline.ts  ← 独立，无 Phaser 依赖
CvColorCode.ts      ← 依赖 Phaser.GameObjects.GameObject
CVBridge.ts         ← 依赖 ObjectIdPipeline (ColorMap)
CvIntegration.ts    ← 依赖 CVBridge、ObjectIdPipeline
```
...
import { initCvIntegration } from '../cv-integration/CvIntegration';
...
- `packages/ballsort/src/game/render/CvColorCode.ts`
- `packages/ballsort/src/game/render/ObjectIdPipeline.ts`
- `packages/ballsort/src/game/cv-bridge/CVBridge.ts`
- `packages/ballsort/src/game/cv-integration/CvIntegration.ts`
- `packages/ballsort/src/game/scenes/Game.ts`（registerCvTintable、initCvIntegration）
- `packages/ballsort/src/game/components/Board.ts`（prepareCvRender、getCvChildren）
- `packages/ballsort/src/game/components/Tube.ts`（prepareCvRender）
- `packages/ballsort/src/game/components/Ball.ts`（getCvTintables、ICvTintableProvider）
