# phaser-cv Demo (Ballsort)

**Official demo for [@ballsort-multi/phaser-cv](../phaser-cv)** — Ballsort game with Color-encoded Instance Segmentation.

```bash
# Run with CV mode (from repo root)
npm run dev:demo
...
Based on Phaser React TypeScript Template. Uses React + Vite for bundling, with a bridge for React ↔ Phaser communication.
...
| `npm run dev:cv` | Launch development server + CV bridge (Python) |
| `npm run cv:check` | Static check for CV integration |
