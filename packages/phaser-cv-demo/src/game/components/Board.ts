...
export class Board extends Phaser.GameObjects.Container implements ICvRenderable, ICvTraversable {
    ...
    /**
     * 返回 cvId → 对象名的映射，用于 CV 检测结果的可读展示。
     * 命名约定：tube1..tubeN, ball_{cvId}, hand, icon, download, liquid, expression
     */
    public getCvIdToLabelMap(): Map<number, string> {
        const map = new Map<number, string>();
        for (const tube of this.tubes) {
            map.set(tube.cvId, `tube${tube.id + 1}`);
            for (const ball of tube.balls) {
                map.set(ball.cvId, `ball_${ball.cvId}`);
            }
        }
        map.set(500, 'hand');
        map.set(501, 'icon');
        map.set(502, 'download');
        map.set(1000, 'liquid');
        map.set(1001, 'expression');
        return map;
    }
