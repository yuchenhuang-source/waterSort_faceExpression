...
import { attachLabels } from '../utils/cvLabelUtils';
import type { ICvSceneAdapter } from '@ballsort-multi/phaser-cv';
...
export class Game extends Scene
{
    ...
    getCvAdapter(): ICvSceneAdapter {
        return {
            ...
            getCvIdToLabelMap: () => this.board.getCvIdToLabelMap(),
            getKeepBlendedPixels: () => getOutputConfigValue<boolean>('cv.keepBlendedPixels', false),
            formatStepSuffix: (res) => {
                ...
                const idToLabel = this.board.getCvIdToLabelMap();
                const labeled = attachLabels(objects, idToLabel);
                const labelStr = labeled.map((o) => o.label).join(', ');
                return ` | 液体${liquidDetected ? '✓' : '-'} 表情${expressionDetected ? '✓' : '-'}${labelStr ? ` | ${labelStr}` : ''}`;
            }
        };
    }
