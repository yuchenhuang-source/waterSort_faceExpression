...
export class CVBridge {
     ...
     * simulator frame, toolbar, or any DOM overlays. When run in the simulator iframe
    ...
    sendFrameAndWait(frameData: PixelFrameData, colorMap?: ColorMap, activeIds?: number[], idToLabel?: Record<string, string>): Promise<CVResponse> {
        return new Promise((resolve, reject) => {
            ...
            if (idToLabel && Object.keys(idToLabel).length > 0) msg.idToLabel = idToLabel;
