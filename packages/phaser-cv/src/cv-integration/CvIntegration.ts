...
import type { ColorMap } from '../render/ObjectIdPipeline.js';
import type { CvTintable, ICvRenderable, ICvTraversable } from '../render/CvColorCode.js';
import { generateColorMap, extractValidPixels } from '../render/ObjectIdPipeline.js';
import { applyCvTintables, collectAllCvIds } from '../render/CvColorCode.js';
import type { CVResponse } from '../cv-bridge/CVBridge.js';
import { isCVModeEnabled, getCVBridge, destroyCVBridge } from '../cv-bridge/CVBridge.js';
import type { ICvSceneAdapter } from './CvSceneAdapter.js';
import { verifyCvIntegration } from './CvIntegrationVerify.js';
...
export function initCvIntegration(scene: Phaser.Scene, options: CvIntegrationOptions): void {
  ...
  verifyCvIntegration(options as ICvSceneAdapter);
  ...
  const captureColorCodedFrame = (): {
    ...
    const keepBlended = getKeepBlendedPixels();
  ...
  const stepOneFrame = async () => {
    ...
    try {
      ...
      const mapOrRecord = getCvIdToLabelMap?.();
      const idToLabel =
        mapOrRecord instanceof Map
          ? Object.fromEntries([...mapOrRecord.entries()].map(([k, v]) => [String(k), v]))
          : mapOrRecord;
